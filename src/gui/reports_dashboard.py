...
class ReportsDashboard:
    ...
    def generate_custom_report(self):
        ...
        tk.Label(dialog, text="Select report type:", font=('Arial', 11)).pack(pady=10)

        report_type = tk.StringVar(value="patient_demographics")
        
        ttk.Radiobutton(dialog, text="Patient Demographics", variable=report_type, value="patient_demographics").pack(anchor='w', padx=30)
        ttk.Radiobutton(dialog, text="Financial Summary", variable=report_type, value="financial_summary").pack(anchor='w', padx=30)

        def show_report():
            dialog.destroy()
            report_name = report_type.get()
            report_text = ""
            try:
                if report_name == "patient_demographics":
                    data = self.db_manager.execute_query("SELECT gender, COUNT(*) as count FROM patients GROUP BY gender")
                    report_text = "Patient Demographics by Gender:\n\n" + "\n".join([f"{row['gender'] or 'Not Specified'}: {row['count']}" for row in data])
                elif report_name == "financial_summary":
                    data = self.db_manager.execute_query("SELECT payment_status, SUM(total_amount) as total, COUNT(*) as count FROM billing GROUP BY payment_status")
                    report_text = "Financial Summary by Payment Status:\n\n" + "\n".join([f"Status: {row['payment_status'].title()}, Count: {row['count']}, Total: ${row['total'] or 0:,.2f}" for row in data])
                else:
                    report_text = "Unknown report type."
                messagebox.showinfo("Custom Report", report_text)
            except Exception as e:
                messagebox.showerror("Report Error", f"Failed to generate report: {e}")

        tk.Button(dialog, text="Generate", command=show_report).pack(pady=20)
